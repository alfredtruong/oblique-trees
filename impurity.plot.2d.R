...
browser()
#load("impurity.plot.2d.crabs.data.detail.50.R") calculated from another machine to just plot it without worrying
#load("impurity.plot.2d.crabs.data.detail.100.R") calculated from another machine to just plot it without worrying
...
#			print(paste("x =",x.index,", y =",y.index))	#just so I can see where the calculation is at
	...
	postscript("impurity.plot.2d.crabs.data.ps",width=10,height=10)
	par(mar=c(1,3,0,0))
	THETA=55
	PHI=55
	COL="lightblue"
	...
	persp(	x=x.grid,
		y=y.grid,
		z=z.grid,
#		main="Perspective plot of sample impurity values for oblique splits",
		xlab="pc2",
		ylab="pc3",
		zlab="split impurity",
		theta=THETA,
